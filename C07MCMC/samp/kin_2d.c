...
void samp_ising_kin_2d(int size, double temp, double bias, int iter, int start, VSLStreamStatePtr s, int size_buf, int* work_int, double* work_dbl, double* m, double* ma, double* u, double* c)
{
    ...
    int(* c_stack)[n*n] = (void*)(work_int + 3*(n*n));
    int(* q_)[n] = (void*)q, (* q_c_)[n] = (void*)q_c, (* q_coor_)[n] = (void*)q_coor;
    ...
    for (int i = 0; i < iter; i++)
    {
        ...
        int q_now = q_[x][y];
        pop_and_push_2d(n, x, y, q_c, q_coor, ctr_c, (void*)c_stack, -5*q_now);
        int 
            x_w = (x + n - 1) % n, x_e = (x + 1) % n,
            y_s = (y + n - 1) % n, y_n = (y + 1) % n;
        pop_and_push_2d(n, x_w, y, q_c, q_coor, ctr_c, (void*)c_stack, -q_now);
        pop_and_push_2d(n, x_e, y, q_c, q_coor, ctr_c, (void*)c_stack, -q_now);
        pop_and_push_2d(n, x, y_s, q_c, q_coor, ctr_c, (void*)c_stack, -q_now);
        pop_and_push_2d(n, x, y_n, q_c, q_coor, ctr_c, (void*)c_stack, -q_now);
